### Introduction
- Gain a solid understanding of computer and processor architecture, including how programs interact with it.
- Learn to disassemble and debug binaries to analyze their machine instructions and purpose.
- Introduce and explore `shellcodes`, a key concept for penetration testers.
...
- First, we can install `pwntools` with the following command (it should be already installed in PwnBox).
```shell-session
...
>>> file = ELF('helloworld')
```
- Now, we can run various `pwntools` functions on it, which we can read more about [here](https://docs.pwntools.com/en/stable/elf/elf.html).
...
- We can write the following `bash` script into `shellcoder.sh` and use it to extract the shellcode if we can't use the first script.
```bash
#!/bin/bash
...
### Debugging Shellcode
- Learn how to debug shellcode using `gdb` when loading machine code directly into memory.
- Option 1: Run shellcode with `loader.py` and attach to its process using `gdb -p PID` (requires the process to remain active).
- Option 2: Build the shellcode into an `ELF` binary and debug it with `gdb` as done earlier in the module.
