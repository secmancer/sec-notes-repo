...
- To print the current Fibonacci number in the `loopFib` loop, we can use the `printf` function from the `libc` library, which automatically handles the conversion of integers to a printable format, avoiding the need to manually convert the number to `ASCII`.
- Before using `printf`, we must import the necessary `libc` functions and specify the `libc` library for dynamic linking when linking the code with `ld`. This allows us to leverage the standard C library functions for more efficient and straightforward printing.
...
### Function Arguments
- We have already discussed how to pass function arguments in the syscall section.
- The same process applies to function arguments.
...
- The `printf` man page also details various print formats. 
- We want to print an integer, so we can use the `%d` format, as follows:
```nasm
global  _start
extern  printf
...
- This requirement is mainly there for processor performance efficiency. 
...
8
```
- As we can see, `printf` made it very easy to print our Fibonacci number without worrying about converting it to the proper format, like we had to with the `write` syscall. 
- Next, we need to go through another example of using external `libc` functions to understand how to call external functions properly.
