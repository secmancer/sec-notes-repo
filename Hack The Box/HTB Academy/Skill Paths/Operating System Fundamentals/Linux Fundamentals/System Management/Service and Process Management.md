...
```shell-session
secmancer@htb[/htb]$ systemctl start ssh
```
...
```shell-session
secmancer@htb[/htb]$ ps -aux | grep ssh

root       846  0.0  0.1  72300  5660 ?        Ss   Mai14   0:00 /usr/sbin/sshd -D
```
- We can also use `systemctl` to list all services.
```shell-session
secmancer@htb[/htb]$ systemctl list-units --type=service
