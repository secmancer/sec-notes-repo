...
- Both share the same features, even syntax and options.
...
- #### Locating TShark
	...
	- Utilizing TShark is very similar to TCPDump in the filters and switches we can use. Both tools utilize BPF syntax. 
	- To read the capture, tshark can be passed the `-r` switch just like in TCPDump, or we can pass the `-P` switch to have tshark print the packet summaries while writing out to a file.
...
```
- #### Applying Filters
- `-f` allows us to apply filters to the capture.
- For instance, we can use `host` mostly, but we do have the option of using almost any filter Wireshark also recognizes
```shell-session
...
- Now that we have spent time learning the art of packet capture from the command line let's spend some time in Wireshark. 
- We will take a few minutes to examine what we are looking at in the output below. 
- #### Wireshark GUI
...
- **Filtering Options in Wireshark**
    - **Capture Filters**:
        ...
        - Useful for reducing the size of captured data.
    - **Display Filters**:
        - Applied **during** or **after** the capture is complete.
        - Allows focusing on specific packets of interest within the captured data.
- **Performance Considerations**
    - Wireshark may struggle with large captures:
        - Longer times to run display or analysis filters.
        - Processing may take **seconds to minutes**, or may not complete at all for very large captures.
- **Best Practices for Large Captures**    
    - Break large `.pcap` files into **smaller chunks** for easier processing.
...
- Think of ports and protocols more like guidelines instead of rigid rules. 
- Ports can be bound and used for different purposes other than what they were originally intended.
- For example, filtering for HTTP will look for key markers that the protocol uses, such as GET/POST requests, and show results from them. 
- Filtering for port 80 will show anything sent or received over that port regardless of the transport protocol.
